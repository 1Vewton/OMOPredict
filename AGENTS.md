...
> 当前仓库处于 **M2 阶段（已完成）**：对标框架（schema/执行器/指标/报告）+ 3 篇真实文献
> 数据集（Voronin 2025 / Isiyaku 2020 / Lim 2020）+ 模型校准闭环全部落地；
> 校准结论：WO₃₋ₓ/Ag/WO₃₋ₓ 吻合良好，Ag 有效参数需上调（界面散射），薄 Ag 渗流为模型边界；
> 下一步 M2.5（NN 代理模型）或 M3（Go 中间层）。
...
**当前进度**：M2 ✅ 完成（框架 + 3 篇数据集 + 模型校准闭环）；下一步 M2.5（NN 代理模型）或 M3（Go 中间层）。
...
*最后更新：M2 完成。每次架构或物理模型变更时，记得同步更新本文件。*
