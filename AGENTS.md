...
│   │   ├── optimize/          # 参数扫描 / 优化（M5）
│   │   ├── neural/            # NN 代理模型（M2.5）
│   │   ├── benchmark/         # 文献对标与误差评估（M2 框架完成，数据集提取中）
...
> 当前仓库处于 **M2 阶段（进行中）**：对标框架完成；三篇真实文献数据集已入库并跑出首份
> 对标报告（WO₃₋ₓ/Ag/WO₃₋ₓ 吻合良好，In₂O₃/Ag/In₂O₃ 与 ITO/Al/Ag/ITO 的偏差指向
> 界面散射与材料常数校准，即 M2.3 目标）；模型校准进行中。
...
**当前进度**：M2（进行中）——框架完成 + 3 篇真实文献数据集入库（Voronin 2025 / Isiyaku 2020 / Lim 2020）；下一步 M2.3 模型校准。
